...
class NuclearParser {
	...
	private auto regexIsotope = regex(`^([a-zA-Z]+)([0-9]+)$`);
}
...
class DecayContext {
	...
	public final void saveIsotopeMasses() {
		previousIsotopeMasses = isotopeMasses.DEEPCOPY;
	}
}
...
class DecayManager {
	...
	public void iteration(DecayContext context) {
		...
		assert( context.isotopeMasses == numberOfIsotopes );
		assert( context.previousIsotopeMasses == numberOfIsotopes );
	...
	private static void calcDecayedRatios(IsotopeInfo isotope, double *decayedIsotopeMasses, double *deltaIsotopeMasses) {
		...
		for( int i = isotope.decayInfosLength-1; i >= 1; i-- ) {
	...
	private double calcDecayEnergyInJoules(IsotopeInfo isotope, double *decayedIsotopeMasses) {
		...
		for( int i = isotope.decayInfosLength-1; i >= 1; i-- ) {
			...
			energyInJoules += calcProducedEnergyOfDecayInJoules(isotope.decayInfos[i], isotopeInfos[destinationIsotope], isotope, decayedMassOfDecayProcess);
...
double calcReleasedEnergyOfAlphaDecayInJoule(double inputMassInKilogram, double atomicMassOfAtomBeforeDecayInAtomicMass) {
	double massWithoutAlphaParticleInAtomicMass = atomicMassOfAtomBeforeDecayInAtomicMass - MASS_HELIUM_IN_ATOMICMASS;
...
// https://en.wikipedia.org/wiki/Beta_decay
// corrected
double calcReleasedEnergyOfBetaPlusDecayInJoule(double atomicMassOfAtomBeforeDecayInAtomicMass, double atomicMassOfAtomAfterDecayInAtomicMass) {
...
// TODO< correct me >
double calcReleasedEnergyOfFissionInJoule(double inputMassInKilogram, double atomicMassOfAtomBeforeDecayInAtomicMass, double numberOfEmittedNeutronsPerDecay) {
	return calcReleasedEnergyOfFissionInJoule(double inputMassInKilogram, double atomicMassOfAtomBeforeDecayInAtomicMass, double numberOfEmittedNeutronsPerDecay);
}
